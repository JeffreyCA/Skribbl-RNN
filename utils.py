...
def augment_strokes(strokes, prob=0.0):
    ...
    for i in range(len(strokes)):
        ...
        if candidate[2] == 0 and prev_stroke[
                2] == 0 and count > 2 and urnd < prob:
...
class DataLoader(object):
    ...
    def random_scale(self, data):
        ...
        x_scale_factor = (np.random.random() -
                          0.5) * 2 * self.random_scale_factor + 1.0
        y_scale_factor = (np.random.random() -
                          0.5) * 2 * self.random_scale_factor + 1.0
        result = np.copy(data)
    ...
    def random_batch(self):
        ...
        idx = np.random.permutation(list(range(0, len(
            self.strokes))))[0:self.batch_size]
    ...
    def pad_batch(self, batch, max_len):
        ...
        for i in range(self.batch_size):
            ...
            result[i, 0, :] = 0
            result[i, 0,
                   2] = self.start_stroke_token[2]  # setting S_0 from paper.
