...
def layer_norm_all(h,
    ...
    with tf.variable_scope(scope):
        ...
        if use_bias:
            beta = tf.get_variable('ln_beta', [4 * num_units],
                                   initializer=tf.constant_initializer(0.0))
    if use_bias:
        return gamma * h + beta
    return gamma * h
...
def layer_norm(x,
    ...
    with tf.variable_scope(scope):
        ...
        gamma = tf.get_variable(
            ...
            initializer=tf.constant_initializer(gamma_start))
        if use_bias:
            beta = tf.get_variable('ln_beta', [num_units],
                                   initializer=tf.constant_initializer(0.0))
...
def super_linear(x,
    ...
    with tf.variable_scope(scope or 'linear'):
        ...
        w = tf.get_variable('super_linear_w', [x_size, output_size],
                            tf.float32,
                            initializer=w_init)
        if use_bias:
            b = tf.get_variable(
  ...
  """
    ...
    def __call__(self, x, state, timestep=0, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            w_full = tf.concat([w_xh, w_hh], 0)
            concat = tf.matmul(concat,
                               w_full)  #+ bias # live life without garbage.
...
class HyperLSTMCell(tf.nn.rnn_cell.RNNCell):
  ...
  """
    ...
    def hyper_norm(self, layer, scope='hyper', use_bias=True):
        ...
        with tf.variable_scope(scope):
            zw = super_linear(self.hyper_output,
                              embedding_size,
                              init_w='constant',
                              weight_start=0.00,
                              use_bias=True,
            ...
            if use_bias:
                zb = super_linear(self.hyper_output,
                                  embedding_size,
                                  init_w='gaussian',
                                  weight_start=0.01,
                                  use_bias=False,
